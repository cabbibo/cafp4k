Shader "FantasyTree/BillboardedFragHSVShader"
{
    ...
    _HSVStart("_HSVStart" , Vector) =(1,1,1)
    _HSVEnd("_HSVEnd" , Vector) =(3,1,1)
    
    _SpriteAlphaHueAdd("_SpriteAlphaHueAdd" , float) = .5
...
TRANSFER_SHADOW(o)
            ...
            { 
                ...
                // the base texture
                float h = lerp(_HSVStart.x,_HSVEnd.x , v.timeCreated );
                float s = lerp(_HSVStart.y,_HSVEnd.y , v.timeCreated );
                float va = lerp(_HSVStart.z,_HSVEnd.z , v.timeCreated );
                col.xyz = hsv( h + spriteCol.a * _SpriteAlphaHueAdd, s, va);//hsv(v.timeCreated * .2 + .75,.5,1);
