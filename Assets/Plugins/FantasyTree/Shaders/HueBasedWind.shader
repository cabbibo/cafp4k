Shader "FantasyTree/BillboardedWind"
{
    ...
    _BaseColorMultiplier("_BaseColorMultiplier" , Range(0,3)) = .5
    _TipColorMultiplier("_TipColorMultiplier" , Range(0,3)) = .5
...
float3 _WindDir = float3(1,0,0);///curlNoise(centerPos);
            float3 worldNor = normalize(mul( unity_ObjectToWorld, float4(v.normal,0)).xyz);
    float3 wind = _WindDir * (sin(_Time.y * 4 + snoise(centerPos) * 10 + .6 *sin(_Time.y * 1.5312)));// (centerPos * .1 + _Time.x);
    wind += _WindDir * (sin(_Time.y * 2+ snoise(centerPos *.1) * 10 + .6 *sin(_Time.y * .9312)));// (centerPos * .1 + _Time.x);


wind  = noise(centerPos * .3 + _Time.y * .3) * .6;
wind += noise(centerPos * 2.5 + _Time.y*2) * .4;

centerPos += wind * .1 * v.texcoord2.z* v.texcoord2.z;
    worldNor  = normalize(worldNor +  (wind-.5) * 4* v.texcoord2.z);
            v.normal = normalize(mul( unity_WorldToObject, float4(worldNor,0)).xyz);
               ...
               // centerPos += wind * .1;
                ...
                fPos = lerp( fPos , float3(fPos.x + sin( falling * 12 +hash(life * 402) * 1012) * .1 , hash(life) * .2 + .1 , fPos.z + cos(falling * 10 + hash(life * 20) * 2012) * .3 ), falling);
...
//o.life = life;
            ...
            // falsely draw shapes
            v.normal.xyz = normalize(v.normal.xyz);//lerp(normalize(v.normal), normalize(v.normal) , fLerp);
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            fixed4 c = tex2D (_SpriteTex, IN.spriteUV ) * lerp( _BaseColor , _TipColor , IN.life);
